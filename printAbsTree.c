...
void print(Cmd* exp, int tabs){
	switch(exp->kind){
		...
		case IF:
			...
			identar(2+tabs);
			print(exp->attr.ifType.cmd,2+tabs);
			if(exp->attr.ifType.list == NULL) break;
			while(exp->attr.ifType.list != NULL) {
				print(exp->attr.ifType.list->cmd, 2+tabs);
		...
		case IFELSE:
			...
			if(exp->attr.elseType.flist != NULL) {
				while(exp->attr.elseType.flist != NULL) {
					print(exp->attr.elseType.flist->cmd, 2+tabs);
		...
		case WHILE:
			identar(tabs);
			printf("while \n");
			printBool(exp->attr.whileType.cond,4+tabs);
			//identar(2+tabs);
			print(exp->attr.whileType.cmd,2+tabs);
			if(exp->attr.whileType.list != NULL) {
				while(exp->attr.whileType.list != NULL) {
					print(exp->attr.whileType.list->cmd, 2+tabs);
